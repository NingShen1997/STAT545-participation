...
```{r}
my_resid <- augment(my_lm)
ggplot(my_resid, aes(.resid)) + geom_freqpoly(binwidth = 0.5)
```
...
```{r}
# default value is null and use the priginal dataset to predict
predict(my_lm) %>% 
  head()

# pass the new data set to do prediction
...
# the years are 1,2,3,.. after shifting the year variable 
```
