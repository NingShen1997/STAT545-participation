...
gapminder %>%
  mutate(gdpBill = (gdpPercap*pop/10^9) %>% round(digits = 2))
...
y = gapminder %>%
  mutate(lifeExp = if(country == "Canada" & year == 1952){
    70
    }else{
      lifeExp} )
 
y[y$country=="Canada" & y$year==1952,] ###doesn't work
```
